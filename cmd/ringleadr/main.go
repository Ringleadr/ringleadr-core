...
import (
	...
	"github.com/Ringleadr/ringleadr-core/internal/Networks"
	"github.com/Ringleadr/ringleadr-core/internal/Nodes"
	"github.com/Ringleadr/ringleadr-core/internal/Overview"
	"github.com/Ringleadr/ringleadr-core/internal/Storage"
...
func main() {
	...
	Logger.Logger().Infof("Starting Agogos in %s mode", agogosMode)
	...
	if agogosMode == "Secondary" {
		...
		if err != nil {
			Logger.Logger().Errorf("Error sending register request to host %s: %s", *connectAddress, err.Error())
		}
	}
	//Logger.Println("Starting front end")
	//go http.ListenAndServe(":14441", http.FileServer(static.HTTP))
	Logger.Logger().Infoln("Starting reporter thread")
	Nodes.StartStatsReporter()
	Logger.Logger().Infoln("Ready to serve")
