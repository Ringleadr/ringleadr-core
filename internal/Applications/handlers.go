...
// CreateApplication Gin handler responsible for creating a new Ringleadr application
...
func DeleteApplication(ctx *gin.Context) {
	...
	if err != nil {
		...
		Logger.Logger().Errorf("Could not remove components for %s: %s", name, err.Error())
	}
...
func deleteApplication(name string) (int, error) {
	...
	if host, err := os.Hostname(); err == nil && host == app.Node || "*" == app.Node {
		...
		go func() {
			...
			Logger.Logger().Errorf("Error deleting implicit network %s: %s", app.Name, err.Error())
		}()
	}
