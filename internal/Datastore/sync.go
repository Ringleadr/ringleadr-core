...
import (
	"fmt"
	"github.com/Ringleadr/ringleadr-core/internal/Components"
...
func syncTick(runtime Containers.ContainerRuntime, mode string, address string) {
	...
	if err != nil {
		...
		Logger.Logger().Errorf("Error getting hostname in sync thread: %s", err.Error())
		return
	}
	...
	apps, err := GetAllApps()
	if err != nil {
		Logger.Logger().Errorf("error getting Applications from datastore in sync thread: %s", err.Error())
	...
	if err != nil {
		Logger.Logger().Errorf("error getting containers from runtime in sync thread: %s", err.Error())
		//We don't explicitly deal with the error here as we will come back around again in 10s and retry
		return
	}
	...
	if err != nil {
		Logger.Logger().Errorf("error getting networks from datastore in sync thread: %s", err.Error())
		//We don't explicitly deal with the error here as we will come back around again in 10s and retry
		return
	}
	...
	if mode == "Primary" {
		...
		if err != nil {
			Logger.Logger().Errorf("Error fetching nodes from sync thread: %s", err.Error())
		}
		...
		nodes, err = GetAllNodes()
		if err != nil {
			Logger.Logger().Errorf("error getting nodes from datastore in sync thread: %s", err.Error())
			//We don't explicitly deal with the error here as we will come back around again in 10s and retry
			return
		}
		apps, err = GetAllApps()
...
func createMissingComponents(apps []Datatypes.Application, containers []*Containers.Container, runtime Containers.ContainerRuntime, hostname string) {
	...
	for _, app := range apps {
			...
			for _, comp := range app.Components {
				...
				for j := 0; j < comp.Replicas; j++ {
					if matchingCont := lookForMatchingContainer("/"+Containers.GetContainerNameForComponent(comp.Name, app.Name, i, j), containers); matchingCont == nil {
						...
						if err != nil {
							Logger.Logger().Errorf("Error starting component %s in app %s: %s", comp.Name, app.Name, err.Error())
				...
				if comp.ScaleThreshold != 0 {
					...
					if int(desiredReplicas) != comp.Replicas {
						...
						for j := comp.Replicas; j < oldReplicas; j++ {
							...
							if err != nil {
								Logger.Logger().Errorf("Could not remove unneeded replica: %s", err.Error())
							}
						}
						shouldSave = true
					}
		...
		if shouldSave {
			err := UpdateApp(&app)
			if err != nil {
				Logger.Logger().Errorf("Error updating %s in application datastore: %s", app.Name, err.Error())
			}
		}
		for _, comp := range app.Components {
			...
			go func(appName string, compName string, stats []Containers.Stats) {
				...
				if err != nil {
					Logger.Logger().Errorf("Error inserting stats for %s: %s", compName, err.Error)
				}
...
func createMissingAppNetworks(app Datatypes.Application, runtime Containers.ContainerRuntime) {
	for _, net := range app.Networks {
		...
		if err != nil {
			Logger.Logger().Errorf("Error fetching network %s from datastore: %s", formatName, err.Error())
		...
		if fetchedNetwork == nil {
			...
			if err != nil {
				Logger.Logger().Infof("Error inserting network %s: %s", formatName, err.Error())
			}
		}
	}
	...
	if err != nil {
		Logger.Logger().Errorf("Error checking for existing implicit network %s: %s", app.Name, err.Error())
		//Hope it's fixed next go round
	} else {
		if !exists {
			err = Containers.GetContainerRuntime().CreateNetwork(app.Name)
			if err != nil {
				Logger.Logger().Errorf("Error creating implicit network for %s: %s", app.Name, err.Error())
			}
		}
	}
}
	...
	for _, cont := range containers {
		...
		if cont.Status == "created" {
			go func(name string) {
				err := runtime.DeleteContainer(name)
				if err != nil {
					Logger.Logger().Errorf("Error deleting container stuck in creating: %s", name, err.Error())
				}
			}(cont.Name)
		}
...
func createMissingNetworks(networks []Datatypes.Network, runtime Containers.ContainerRuntime) {
	for _, net := range networks {
		...
		if err != nil {
			Logger.Logger().Errorf("error checking network %s in runtime from sync thread. Error was: %s", net.Name, err.Error())
			continue
		}
		if !exists {
			go func(name string) {
				...
				if err != nil {
					Logger.Logger().Errorf("Error creating network %s: %s", name, err.Error())
				}
			}(net.Name)
		}
	}
...
func assignAppsWithNoNodes(apps *[]Datatypes.Application, distribution map[string]int) {
	for _, app := range *apps {
		...
		if selectedNode != "" {
			...
			err := UpdateApp(&app)
			if err != nil {
				Logger.Logger().Errorf("Error assigning node to app: %s", err.Error())
			}
		}
	}
}
...
func checkForInactiveNodes(nodes []Datatypes.Node, hostname string) {
	for _, node := range nodes {
		...
		if oldStatus != node.Active {
			err := UpdateNode(&node)
			if err != nil {
				Logger.Logger().Errorf("Error updating node status in sync thread: %s", err.Error())
			}
		}
	}
}
...
func displayErrorForAppsOnInactiveNodes(apps []Datatypes.Application, nodes []Datatypes.Node) {
	for _, app := range apps {
		...
		if shouldSave {
			err := UpdateApp(&app)
			if err != nil {
				Logger.Logger().Errorf("Error saving inactive node message to application: %s", err.Error())
			}
		}
	}
}
