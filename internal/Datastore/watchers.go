...
func watchStorage(coll *mgo.Collection) {
	var insertFunc = func(changeDoc bson.M) {
		...
		if err != nil {
			Logger.Logger().Errorf("Error watching storage: ", err)
		}
	}
...
func watchNetworks(coll *mgo.Collection) {
	var insertFunc = func(changeDoc bson.M) {
		...
		if err != nil {
			Logger.Logger().Errorf("Error watching networks: ", err)
		}
	}
...
func watchGeneralCollection(coll *mgo.Collection, funcs map[string]func(changeDoc bson.M)) {
	...
	for {
		...
		err = stream.Err()
		if err != nil {
			Logger.Logger().Errorf("error whilst watching stream: %s", err.Error())
		}
	}
}
...
func unMarshalIntoApp(m bson.M, app *Datatypes.Application) {
	...
	if err != nil {
		Logger.Logger().Errorf("Error getting bson document: ", err, bsonBytes)
		return
	}
	err = bson.Unmarshal(bsonBytes, &app)
	if err != nil {
		Logger.Logger().Errorf("Error unmarshalling bson: ", err)
	}
}
