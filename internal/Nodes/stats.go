...
func StartStatsReporter() {
	go func() {
		...
		for {
			...
			if err != nil {
				Logger.Logger().Errorf("Error getting hostname in reporter thread: %s", err.Error())
				continue
			}
			...
			if len(errs) > 0 {
				for _, err := range errs {
					if _, ok := errorMap[err.Error()]; !ok {
						Logger.Logger().Errorf("Could not get all stats in reporter thread: %s", err.Error())
			...
			if err != nil {
				Logger.Logger().Errorf("Error updating stats in DB: %s", err.Error())
			}
		}
	}()
}
